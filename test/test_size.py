...
Hierarchy = collections.namedtuple('Hierarchy', ['collections', 'folders', 'files'])
...
def hierarchy(admin, fsAssetstore):
    ...
    file1 = File().createFile(
        name='F1', creator=admin, folder=f1, size=1, assetstore=fsAssetstore, mimeType='text/csv')
    file2 = File().createFile(
        name='F2', creator=admin, folder=f2, size=10, assetstore=fsAssetstore, mimeType='text/csv')

    yield Hierarchy(collections=(c1, c2), folders=(f1, f2), files=(file1, file2))
...
def testMoveFileToSubfolder(server, admin, hierarchy):
    ...
    resp = server.request(
        path='/file/%s' % hierarchy.files[0]['_id'], method='PUT',
        user=admin, params={
...
def testDeleteFileUpdatesSize(server, admin, hierarchy):
    resp = server.request(path='/file/%s' % hierarchy.files[0]['_id'], method='DELETE', user=admin)
    assertStatusOk(resp)
